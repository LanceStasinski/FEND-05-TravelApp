...
const getImageCity = async (coords) => {
  const response = await fetch(`https://pixabay.com/api/?key=${imageKey}&q=${coords.geonames[0].name}+${coords.geonames[0].countryName}&image_type=photo&orientation=horizontal&category=places&safesearch=true`);
  try {
...
const getImageNotUSA = async (coords) => {
